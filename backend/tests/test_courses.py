...
@pytest.mark.parametrize('search_params, expected_course_titles', [
...
def test_search_filter_course(api_client, search_params, expected_course_titles):
    ...
    java_course = CourseFactory.create(title='Java Developer', level=3, category=[java_category, programming_category])
    python_course = CourseFactory.create(title='Python Developer', level=2, category=[python_category, programming_category])
