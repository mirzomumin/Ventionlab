...
def test_course_api_create(api_client):
    ...
    created_course_categories = Course.objects.get(
        id=created_course_id
    ).category.values_list("id", flat=True)
    assert set(created_course_categories) == {
        python_category.id,
        programming_category.id,
    }
